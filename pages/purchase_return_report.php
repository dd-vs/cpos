...
navbar_user(4);
navbar_report(1); 
$query="SELECT ti_purchase_return.*,DATE_FORMAT(ti_purchase_return.ret_date,'%d/%m/%Y') as ret_date,ti_suppllier.name,ti_purchase_invoice.invoice_num FROM `ti_purchase_return` left join ti_purchase_invoice on ti_purchase_invoice.invoice_id=ti_purchase_return.invoice_id left join ti_suppllier on ti_suppllier.id=ti_purchase_invoice.supplier_id ";
	$s=$conn->query($query);
 $s->setfetchmode(PDO::FETCH_ASSOC);
 $query33="SELECT sum(ti_purchase_return.ret_amt) as ret_amt from ti_purchase_return where ti_purchase_return.ret_date between (CURRENT_DATE - INTERVAL 30 DAY ) and now() and ti_purchase_return.IsActive=1";
...
<div class="report-container">
    <h2 class="margin-top-10">Purchase Return Report<span class="btn btn-primary" onclick="printInv().print();"><i class="fa fa-print" aria-hidden="true"></i> </span></h2>
    <div class="report-head">
        <form class="" id="" action="purchase_return_report.php" method="post">
            <div class="row">
                <div class="col-md-3">
	...
	$query="SELECT ti_purchase_return.*,DATE_FORMAT(ti_purchase_return.ret_date,'%d/%m/%Y') as ret_date,ti_suppllier.name,ti_purchase_invoice.invoice_num FROM `ti_purchase_return` left join ti_purchase_invoice on ti_purchase_invoice.invoice_id=ti_purchase_return.invoice_id left join ti_suppllier on ti_suppllier.id=ti_purchase_invoice.supplier_id where ti_purchase_return.ret_date between  '$f 00:00:00' and '$t 23:59:59'";
	$s=$conn->query($query);
 $s->setfetchmode(PDO::FETCH_ASSOC);
 $query33="SELECT sum(ti_purchase_return.ret_amt) as ret_amt from ti_purchase_return where ti_purchase_return.ret_date between  '$f 00:00:00' and '$t 23:59:59' and ti_purchase_return.IsActive=1";
				$s33=$conn->query($query33);
				$r33=$s33->fetch();
 ...
 $i=1; while($r=$s->fetch()){ ?>
	...
	$query="SELECT ti_purchase_return.*,DATE_FORMAT(ti_purchase_return.ret_date,'%d/%m/%Y') as ret_date,ti_suppllier.name,ti_purchase_invoice.invoice_num FROM `ti_purchase_return` left join ti_purchase_invoice on ti_purchase_invoice.invoice_id=ti_purchase_return.invoice_id left join ti_suppllier on ti_suppllier.id=ti_purchase_invoice.supplier_id where ti_purchase_invoice.invoice_num='$f'";
	$s=$conn->query($query);
$s->setfetchmode(PDO::FETCH_ASSOC);
$query33="SELECT sum(ti_purchase_return.ret_amt) as ret_amt from ti_purchase_return left join ti_purchase_invoice on ti_purchase_invoice.invoice_id=ti_purchase_return.invoice_id
where ti_purchase_invoice.invoice_num='$f' and ti_purchase_return.IsActive=1";
				$s33=$conn->query($query33);
				$r33=$s33->fetch();
$i=1; while($r=$s->fetch()){ ?>
                        ...
                        // [left, top, right, bottom] or [horizontal, vertical] or just a number for equal margins
						  ...
						  header: {
							...
							columns: [
                                        [
                                            {columns:[
                                                    {width: 130, text: 'PurchaseReturnReport From', fontSize: 10, bold: true, alignment: 'left'},
