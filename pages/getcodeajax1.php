<?php
  ...
  if(isset($_GET['p']) && $_GET['p']=='getitem') {
	...
	$q1="SELECT count(ti_sale_return.id)  as number FROM `ti_sale_return` left join ti_sale_invoice on ti_sale_return.invoice_id=ti_sale_invoice.invoice_id WHERE ti_sale_invoice.invoice_id='$a'";
...
if($number<=0){
		...
		$query="SELECT ti_sale_item.id as saleid,ti_sale_invoice.invoice_num as invnum,ti_product.name,ti_product.id as pro,master_tax.tax_percent as cgst,ti_sale_item.sell_price as rate,ti_sale_item.* FROM `ti_sale_item` left join ti_sale_invoice
 on ti_sale_item.invoice_id=ti_sale_invoice.invoice_id left join ti_product on ti_product.id=ti_sale_item.product_id left join master_tax on master_tax.id=ti_product.cgst
 WHERE ti_sale_invoice.invoice_id='$a' and ti_sale_item.IsActive=1";
		$val1=$conn->query($query);
		$val1->setfetchmode(PDO::FETCH_ASSOC);
		$query3="SELECT ti_sale_item.id as saleid,ti_sale_invoice.invoice_num as invnum,ti_product.name,ti_product.id as pro,master_tax.tax_percent as sgst,ti_sale_item.sell_price as rate,ti_sale_item.* FROM `ti_sale_item` left join ti_sale_invoice
 on ti_sale_item.invoice_id=ti_sale_invoice.invoice_id left join ti_product on ti_product.id=ti_sale_item.product_id left join master_tax on master_tax.id=ti_product.sgst
 WHERE ti_sale_invoice.invoice_id='$a' and ti_sale_item.IsActive=1";
		$val33=$conn->query($query3);
		$val33->setfetchmode(PDO::FETCH_ASSOC);
...
<td class="no-screen"> <input type="hidden" class="changecgst" name="changecgst[]" id="changecgst" value=""></td><!--  cgst tax amount of changed qty -->
  ...
  <td class="no-screen"> <input type="hidden" class="saleid" name="saleid[]" id="saleid" value="<?php echo $v1['saleid'];?>"> </td> <td class="no-screen"> <input type="hidden" class="grossamt" name="grossamt[]" id="grossamt" value="<?php echo $grossamt;?>"> </td> <td class="no-screen"> <input type="hidden" class="grossamt" name="grossamt[]" id="grossamt" value="<?php echo $grossamt;?>"> </td><!--  grossamt of changed qty -->
		...
		<?php $i++;} }
			...
			where  ti_sale_invoice.invoice_id='$a'";
...
$query1="select count(ti_sale_item.id) as num, master_tax.tax_percent as sgst from ti_sale_item left join ti_sale_invoice on ti_sale_item.invoice_id=ti_sale_invoice.invoice_id left join ti_product on ti_product.id=ti_sale_item.product_id left join master_tax on master_tax.id=ti_product.sgst where ti_sale_invoice.invoice_id='$a'";
		...
		$q23="SELECT ti_sale_return_item.return_inv_id as xnum FROM `ti_sale_return_item` left join ti_sale_return on ti_sale_return_item.return_inv_id=ti_sale_return.id left join ti_sale_invoice on ti_sale_return.invoice_id=ti_sale_invoice.invoice_id
			...
			$qn="SELECT ti_sale_return_item.ret_qty as ret FROM `ti_sale_return_item` left join ti_sale_return on ti_sale_return_item.return_inv_id=ti_sale_return.id left join ti_sale_item on ti_sale_return.invoice_id=ti_sale_item.invoice_id
			where ti_sale_return_item.return_inv_id='$xnum' and  ti_sale_return_item.product_id='".$v1['pro']."' and ti_sale_item.id='".$v1['saleid']."' ";
			$qn22=$conn->query($qn);
