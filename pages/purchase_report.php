...
navbar_user(4);
navbar_report(1);
$query="SELECT ti_purchase_invoice.*,DATE_FORMAT(pur_date,'%d/%m/%Y') as purdate,ti_suppllier.name FROM `ti_purchase_invoice` 
				left join ti_suppllier on ti_suppllier.id=ti_purchase_invoice.supplier_id  left join ti_transport on ti_transport.invoice_id=ti_purchase_invoice.invoice_id
				WHERE  ti_purchase_invoice.IsHidden=10  and ti_purchase_invoice.cash_credit=10 and ti_purchase_invoice.pur_date between (CURRENT_DATE - INTERVAL 30 DAY ) and now() and ti_purchase_invoice.IsActive=1 order by ti_purchase_invoice.pur_date";
				$s=$conn->query($query);
				...
				WHERE  ti_purchase_invoice.IsHidden=10  and ti_purchase_invoice.cash_credit=10 and ti_purchase_invoice.pur_date between (CURRENT_DATE - INTERVAL 30 DAY ) and now() and ti_purchase_invoice.IsActive=1 ";
				$s33=$conn->query($query33);
				$r33=$s33->fetch();

				$query1="SELECT ti_purchase_invoice.*,DATE_FORMAT(pur_date,'%d/%m/%Y') as purdate,ti_suppllier.name FROM `ti_purchase_invoice` 
				left join ti_suppllier on ti_suppllier.id=ti_purchase_invoice.supplier_id  left join ti_transport on ti_transport.invoice_id=ti_purchase_invoice.invoice_id
				WHERE ti_purchase_invoice.IsHidden=10  and ti_purchase_invoice.cash_credit=11 and ti_purchase_invoice.pur_date between (CURRENT_DATE - INTERVAL 30 DAY ) and now() and ti_purchase_invoice.IsActive=1 order by ti_purchase_invoice.pur_date";
				...
				WHERE  ti_purchase_invoice.IsHidden=10  and ti_purchase_invoice.cash_credit=11 and ti_purchase_invoice.pur_date between (CURRENT_DATE - INTERVAL 30 DAY ) and now() and ti_purchase_invoice.IsActive=1 ";
...
<div class="report-container">
    <h2 class="margin-top-10">Purchase Invoice Report <span class="btn btn-primary" onclick="printInv().print();"><i class="fa fa-print" aria-hidden="true"></i> </span></h2>
     ...
     if(isset($_POST['searchbydate'])){
                                               ...
                                               if (isset($_POST['inv_num']) && !empty($_POST['inv_num']) ) {
												   ...
												   unset($_POST['select']);
                                                      ...
                                                      $condition .= " AND ti_purchase_invoice.invoice_num='$num' ";
                                               ...
                                               if (isset($_POST['name']) && !empty($_POST['name']) ) {
												   ...
												   unset($_POST['select']);
                                                      $condition .= " and ti_suppllier.name like '%$name%' ";
                                              } 
				...
				if($post=='purchase')
				{
					 $condition .= "AND ti_purchase_invoice.cash_credit='10' AND ti_purchase_invoice.pur_date between '$f 00:00:00' and '$t 23:59:59' ";
					}
					else if($post=='sales'){
						$condition .= "AND ti_purchase_invoice.cash_credit='11' AND ti_purchase_invoice.pur_date between '$f 00:00:00' and '$t 23:59:59' ";
						}
					else if($post=='salpur'){
						$condition .= "AND ti_purchase_invoice.pur_date between '$f 00:00:00' and '$t 23:59:59' ";
						}
                                                     
                                              }
                                              $query="SELECT ti_purchase_invoice.*,DATE_FORMAT(pur_date,'%d/%m/%Y') as purdate,ti_suppllier.name FROM `ti_purchase_invoice` 
				left join ti_suppllier on ti_suppllier.id=ti_purchase_invoice.supplier_id  left join ti_transport on ti_transport.invoice_id=ti_purchase_invoice.invoice_id
				WHERE  ti_purchase_invoice.IsHidden=10  {$condition}  and ti_purchase_invoice.IsActive=1 order by ti_purchase_invoice.pur_date";
				$s=$conn->query($query);
			
				$res2=$s->fetchAll(PDO::FETCH_ASSOC);
				$query3="SELECT count(ti_purchase_invoice.invoice_num) as num,sum(ti_purchase_invoice.amt) as totamt from ti_purchase_invoice 
				left join ti_suppllier on ti_suppllier.id=ti_purchase_invoice.supplier_id WHERE ti_purchase_invoice.IsHidden=10  {$condition}
				 and ti_purchase_invoice.IsActive=1";
				...
				$r3=$s33->fetch();
					...
					<input type="hidden" id="invno" value="<?php if(isset($_POST['inv_num'])){ echo $_POST['inv_num']; } ?>">
...
//alert(myvar);
'<%Session["temp"] = "' + myvar +'"; %>';
popup("purchaseinvoiceedit.php?temp=" + myvar);
}
                        ...
                        // [left, top, right, bottom] or [horizontal, vertical] or just a number for equal margins
						  ...
						  header: {
							...
							columns: [
                                        [
                                            {columns:[
                                                    {width: 130, text: 'PurchaseInvoiceReport From', fontSize: 10, bold: true, alignment: 'left'},
                                            ...
                                            {columns:[
                                                    {width: 130, text: 'Seller Name', fontSize: 10, bold: true, alignment: 'left'},
