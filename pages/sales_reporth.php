...
<div class="report-container">
    ...
    <div class="report-body">
        <table class="table default-table report-table">
          ...
          if(isset($_POST['searchbydate'])){
				...
				$query="SELECT ti_sale_item.*,ti_sale_invoice.*,ti_customer.name,ti_product.name as item_name,master_unit.unit_name FROM `ti_sale_invoice` 
				left join ti_customer on ti_customer.id=ti_sale_invoice.cust_id left join ti_sale_item on ti_sale_item.invoice_id=ti_sale_invoice.invoice_id 
				left join ti_product on ti_product.id=ti_sale_item.product_id left join ti_transport on ti_transport.invoice_id=ti_sale_invoice.invoice_id 
				 left join master_unit on master_unit.id=ti_product.unit_id
				WHERE  ti_sale_invoice.IsHidden=11  and ti_sale_invoice.cash_credit=10 and ti_sale_invoice.sale_date between '$f 00:00:00' and '$t 23:59:59' and ti_sale_item.IsActive=1";
				$s=$conn->query($query);
				...
				WHERE  ti_sale_invoice.IsHidden=11  and ti_sale_invoice.cash_credit=10 and ti_sale_invoice.sale_date between '$f 00:00:00' and '$t 23:59:59' and ti_sale_invoice.IsActive=1";
				$s33=$conn->query($query33);
				$r33=$s33->fetch();

				$query1="SELECT ti_sale_item.*,ti_sale_invoice.*,ti_customer.name,ti_product.name as item_name,master_unit.unit_name FROM `ti_sale_invoice` 
				left join ti_customer on ti_customer.id=ti_sale_invoice.cust_id left join ti_sale_item on ti_sale_item.invoice_id=ti_sale_invoice.invoice_id 
				left join ti_product on ti_product.id=ti_sale_item.product_id left join ti_transport on ti_transport.invoice_id=ti_sale_invoice.invoice_id 
				 left join master_unit on master_unit.id=ti_product.unit_id
				WHERE ti_sale_invoice.IsHidden=11  and ti_sale_invoice.cash_credit=11 and ti_sale_invoice.sale_date between '$f 00:00:00' and '$t 23:59:59' and ti_sale_item.IsActive=1";
				$s1=$conn->query($query1);
				...
				WHERE  ti_sale_invoice.IsHidden=11  and ti_sale_invoice.cash_credit=11 and ti_sale_invoice.sale_date between '$f 00:00:00' and '$t 23:59:59' and ti_sale_invoice.IsActive=1";
	...
	$query="SELECT ti_sale_item.*,ti_sale_invoice.*,ti_customer.name,ti_product.name as item_name,master_unit.unit_name FROM `ti_sale_invoice`
	 ...
	 left join ti_product on ti_product.id=ti_sale_item.product_id left join ti_transport on ti_transport.invoice_id=ti_sale_invoice.invoice_id 
	  left join master_unit on master_unit.id=ti_product.unit_id
	 WHERE ti_sale_invoice.IsHidden=11 and ti_sale_invoice.invoice_num='$f' and ti_sale_item.IsActive=1";
	$s=$conn->query($query);
$result2=$s->fetchAll(PDO::FETCH_ASSOC);
$query33="SELECT count(ti_sale_invoice.invoice_num) as num ,sum(ti_sale_invoice.amt) as totamt  from ti_sale_invoice left join ti_sale_item on ti_sale_item.invoice_id=ti_sale_invoice.invoice_id 
				WHERE  ti_sale_invoice.IsHidden=11   and ti_sale_invoice.invoice_num='$f' and ti_sale_item.IsActive=1";
				$s33=$conn->query($query33);
				$r33=$s33->fetch();
