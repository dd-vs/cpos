...
$echeck1="select count(item_code) as code from ti_product where item_code='$sku'";
  ...
  if($s['code']!=0)
   ...
   else if(isset($_GET['q']) && $_GET['q']=='get3')
   {
	 $sku=isset($_POST['uname']) ? $_POST['uname']:'';
	 $date=isset($_POST['date']) ? $_POST['date']:'';
	 $show_salary_month = DateTime::createFromFormat('m/Y', $date)->format('Y-m');
	 $newdate = date("Y-m-00",strtotime($show_salary_month));
	 
$echeck1="select count(id) as code from ti_salary_issue  where salary_month='$newdate' and emp_id='$sku'";
  $r11=$conn->query($echeck1);
			$r11->setfetchmode(PDO::FETCH_ASSOC);
			while($s=$r11->fetch())
			{
