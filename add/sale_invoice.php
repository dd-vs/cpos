...
if($selectsale==0){
		$query="select max(ti_sale_invoice.invoice_num)+1 as inv_no from ti_sale_invoice where ti_sale_invoice.IsHidden=10 and ti_sale_invoice.IsActive=1";
	...
	$ae2=$re->fetch();
	$ide=isset($_POST['cust_id']) ? $_POST['cust_id']:'1';
...
if(isset($_POST['proname1']) && count($_POST['proname1']) >0) {
				...
				foreach($_POST['proname1'] as $row) {
             	...
             	//if product not exists
                      ...
                      //STOCK PART
						...
						$append_query .="('$lid','$id','$rate','$dis','$new','$cgst','$sgst',1),";
	...
	$ae=$re->setfetchmode(PDO::FETCH_ASSOC);
	$ae2=$re->fetch();
	$ide11=isset($_POST['cust_id']) ? $_POST['cust_id']:'1';
